...
`budui-meta-skill` owns the complete public release path. Do not require or invoke a separate publisher skill after this package is selected.
...
4. bundled Budui profile/QR assets and idempotent README block injection
...
Prepare LICENSE, README and Budui profile locally without GitHub writes:
...
- `--private`
- `--no-sync-local`
- `--skip-qiaomu-profile` only for explicitly non-Budui packages
